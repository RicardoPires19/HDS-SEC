...
public class ClientLibrary extends UnicastRemoteObject implements Client{
	...
	@Override
	public String checkAccount(PublicKey pubKey, String nonce,  byte[] hmac) throws AuthenticationException{
		...
		byte[] Hmac = null;
		try {
			Hmac = macVerifier.createHmac(serverReply,sc.getSecretKey() );
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		return serverReply + ", HMAC: " + Hmac;
	}
	...
	@Override
	public String receiveAmount(PublicKey pubKey, int id, String nonce, byte[] hmac) throws Exception {
		String concatenation = nonce + pubKey + id;
				
		if(db.checkNonce(nonce, pubKey.toString())){
			return macVerifier.createHmac("This message has already been received"+nonce, sc.getSecretKey()) +"This message has already been received" + nonce;
		}
		if(!verifySession(pubKey))
			return macVerifier.createHmac("Not in Session"+nonce, sc.getSecretKey())+"Not in Session";
		try {
			macVerifier.verifyHMAC(hmac, ks.getKey(pubKey.toString(), PASSWORD), concatenation);
		} catch (Exception e) {
			throw new AuthenticationException("Could not authenticate");
		...
		return macVerifier.createHmac("ACK"+nonce, sc.getSecretKey())+ "ACK" + nonce;
	}	
	...
	public String audit(PublicKey pubKey,String audited, String nonce, byte[] signature) throws RemoteException {
		...
		return serverReply + ", Signature:" + sig;
	}
